...
class ProfileScreen extends Component {
  ...
  static navigationOptions = ({ screenProps, navigation }) => {
    ...
    return {
      //<Text style={{ fontWeight: 'bold', color:'#2db2e6'  }}>
      headerTitleStyle: {
       fontWeight: 'bold', color:'#f68d1f', 
      },
      headerTitle: IMLocalized('προφίλ'),
  ...
  constructor(props) {
    ...
    const shouldAddFriend = (this.otherUser) ? !this.props.friends.find(friend => friend.id == this.otherUser.id) : false;
    this.state = {
      profilePosts: null,
      isCameraOpen: false,
  ...
  componentDidMount() {
    ...
    this.friendshipManager = new FriendshipManager(this.props.users, false, this.onFriendshipsRetrieved);
    if (this.otherUser && this.otherUser.id != this.props.user.id) {
      let profileUserID = this.otherUser.id;
      this.currentProfileFeedUnsubscribe = firebasePost.subscribeToProfileFeedPosts(profileUserID, this.onProfileFeedUpdate);
      this.currentUserUnsubscribe = firebaseUser.subscribeCurrentUser(profileUserID, this.onCurrentUserUpdate);
      this.setState({
        loading: true
      });
      this.friendshipManager.fetchFriendships(this.otherUser.id);  
    } else {
      this.currentProfileFeedUnsubscribe = firebasePost.subscribeToProfileFeedPosts(this.props.user.id, this.onProfileFeedUpdate);
      this.currentUserUnsubscribe = firebaseUser.subscribeCurrentUser(this.props.user.id, this.onCurrentUserUpdate);
      this.friendshipManager.fetchFriendships(this.props.user.id);  
      this.setState({
        profilePosts: this.feedManager.hydratePostsWithReduxReactions(this.props.currentUserFeedPosts),
        loading: true,
      });
    }
  }
  ...
  onFriendshipsRetrieved = (mutualFriendships, inboundFriendships, outboundFriendships) => {
    this.setState({
      loading: false,
  ...
  onProfileFeedUpdate = (profilePosts) => {
    this.setState({
      profilePosts: this.feedManager.hydratePostsWithReduxReactions(profilePosts),
      loading: false
    })
  }
  ...
  startUpload = async source => {
    ...
    firebaseStorage.uploadFileWithProgressTracking(
      ...
      error => {
        self.setState({ uploadProgress: 0 });
        alert(
          IMLocalized('Oops! An error occured while trying to update your profile picture. Please try again.'),
        );
        console.log(error);
      },
    );
  ...
  removePhoto = async () => {
    ...
    if (res.success) {
      ...
      alert(
        IMLocalized('Oops! An error occured while trying to remove your profile picture. Please try again.'),
      );
    }
  };
  ...
  onAddFriend = () => {
    ...
    firebaseFriendship.addFriendRequest(this.props.user, this.otherUser, true, false, true, ({ success, error }) => {
      if (error) {
        alert(error);
        ...
        const detectedFriendship = friendships.find(friendship => friendship.user.id == newFriendId && friendship.type == FriendshipConstants.FriendshipType.reciprocal);
  ...
  render() {
    ...
    let mainButtonTitle = IMLocalized('Επεξεργασία Προφίλ');
    ...
    if (this.otherUser) {
      mainButtonTitle = IMLocalized('        Στείλε Μήνυμα  ');
      if (this.state.shouldAddFriend) {
        mainButtonTitle = IMLocalized('   Πρόσθεσε Γείτονα');
      }
    }

    const initialCountDisplay = 6;
    const displaySubButton = (this.state.friends && this.state.friends.length > initialCountDisplay);
    const friends = (this.state.friends) ? this.state.friends.slice(0, initialCountDisplay) : null;
    ...
    return (
      <Profile
        ...
        subButtonTitle={IMLocalized('Δες όλους τους γείτονες')}
