...
class CreatePostScreen extends Component {
  static navigationOptions = ({ screenProps, navigation }) => {
    ...
    return {
      headerTitle: 'νέα ανάρτηση',
      //headerTitle: 'Create Post',
      headerRight: params.isPosting ? (
        <ActivityIndicator style={{  margin: 10 }} size="small" />
      ) : (

         <TextButton style={{padding:10, color:'white', backgroundColor: '#f47554', position:'relative', left:0, top:0,  zIndex: 100,  marginRight: 12,justifyContent: 'center', alignItems: 'center' }} onPress={params.onPost}>
              Δημοσίευση 
        </TextButton>
),
  ...
  onPost = async () => {
    ...
    self.setState(
      ...
      async () => {
        if (self.state.post && self.state.post.postMedia && self.state.post.postMedia.length === 0) {
          await firebasePost
            .addPost(
              self.state.post,
              friendshipUtils.followerIDs(this.props.friendships, this.props.friends, false),
              self.props.user
            );
