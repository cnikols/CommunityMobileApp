  ...
  return commentsRef.orderBy('createdAt', 'asc').onSnapshot(querySnapshot => {
    ...
    comments = comments.map(async comment => {
      ...
      const promiseComment = {
        ...
        lastName: userDoc.data().lastName || userDoc.data().fullname,
...
export const getUserReactions = (userId, callback) => {
  return reactionsRef
    ...
    .onSnapshot(querySnapshot => {
      ...
      const reactions = querySnapshot._docs.map(doc => {
        const temp = doc.data();
        temp.id = doc.id;
        return temp;
      });
...
export const addComment = async (comment, commentAuthor, post, followEnabled) => {
  try {
    ...
    notificationManager.sendPushNotification(
      ...
      commentAuthor.firstName + " " + IMLocalized("σχολίασε την ανάρτησή σου"),
      "social_comment",
...
export const handleReaction = async (reaction, user, post, followEnabled, users = []) => {
  try {
    ...
    if (documentSnapshots.docs.length > 0) {
      if (followEnabled || reaction == null) {
        ...
        documentSnapshots._docs.forEach(async docRef => {
      ...
      // Send push notification to author
      const message = (followEnabled) ? (user.firstName + " " + IMLocalized("ενδιαφέρθηκε για την ανάρτησή σου")) : (user.firstName + " " + IMLocalized("αντέδρασε στην ανάρτησή σου"));
      notificationManager.sendPushNotification(
        post.author,
        IMLocalized("Instamobile"),
        message,
        "social_reaction",
        { fromUser: user, reaction }
      );
    }
    if (followEnabled) {
      updateReactionsCountForFollowers(post);
    } else {
      updateReactionsCountForFriends(post, users);
    }
  } catch (error) {
    console.log(error);
  }
...
export const updateReactionsCountForFollowers = async (post) => {
  ...
  const allReactions = await reactionsRef
...
const updateReactionsCountForFriends = async (post, users) => {
  ...
  const manager = new FriendshipManager(users, false, (friendships, inbound, outbound) => {
...
const updateCommentCountOnAllTimelines = async (comment) => {
  ...
  // We fetch all users who follow the author of the post and update their timelines
  const unsubscribe = firebaseFriendship.subscribeToInboundFriendships(comment.authorID, inboundFriendships => {
    const inboundUserIDs = inboundFriendships.map(friendship => friendship.user1);
    ...
    allUserIDsToBeUpdated.forEach(userID => {
      const otherUserMainFeedRef = firebase
        ...
        .collection('main_feed')
        .doc(comment.postID)
      batch.set(otherUserMainFeedRef, {
...
const updateCommentCountOnAllFriendsTimelines = async (comment) => {
  ...
  const unsubscribeInbound = firebaseFriendship.subscribeToInboundFriendships(comment.authorID, inboundFriendships => {
    const unsubscribeOutbound = firebaseFriendship.subscribeToOutboundFriendships(comment.authorID, outboundFriendships => {
      const inboundUserIDs = inboundFriendships.map(friendship => friendship.user1);
