...
import { firebasePost } from '../../feed/firebase';
...
export const addFriendRequest = (fromUser, toUser, persistFriendshipsCounts, extendFollowers, enableFeedUpdates, callback) => {
  ...
  friendshipsRef
    ...
    .then(() => {
      ...
      var notificationBody = fromUser.firstName + " " + fromUser.lastName + " " + (extendFollowers ? IMLocalized("just followed you.") : IMLocalized("sent you a friend request."));
...
export const cancelFriendRequest = (currentUserID, toUserID, persistFriendshipsCounts, enableFeedUpdates, callback) => {
...
export const unfriend = async (currentUserID, toUserID, persistFriendshipsCounts, enableFeedUpdates, callback) => {
  ...
  if (enableFeedUpdates) {
    ...
    cancelFriendRequest(currentUserID, toUserID, persistFriendshipsCounts, enableFeedUpdates, response => {
      callback(response)
    });
  } else {
    cancelFriendRequest(currentUserID, toUserID, persistFriendshipsCounts, enableFeedUpdates, _response => {
      cancelFriendRequest(toUserID, currentUserID, persistFriendshipsCounts, enableFeedUpdates, response => {
        callback(response)
      });
    });
  }
...
export const updateFriendshipsCounts = async (userID) => {
  // inbound
  const inbound =
