...
class IMAllFriendsScreen extends Component {
  ...
  constructor(props) {
    ...
    this.includeReciprocal = this.props.navigation.getParam('includeReciprocal');
  ...
  componentDidMount() {
    this.friendshipTracker = new FriendshipTracker(this.context.store, this.props.user.id, true, this.followEnabled, true);

    const vieweeID = (this.otherUser) ? this.otherUser.id : this.props.user.id;
    this.friendshipManager = new FriendshipManager(this.props.users, this.followEnabled, this.onFriendshipsRetrieved);
  ...
  onFriendshipsRetrieved = (reciprocalFriendships, inboundFriendships, outboundFriendships) => {
    ...
    this.setState({
      friendships: this.hydrateFriendshipStatusesForCurrentUser(finalFriendships),
  ...
  render() {
    ...
    if (this.state.friendships && this.state.loggedInUserFriendships && this.state.loggedInUserFriendships != this.props.friendships) {
      // If the current user follows changed since the last rerender (e.g. followed someone)
      this.setState({
        isLoading: false,
        loggedInUserFriendships: this.props.friendships,
        friendships: this.hydrateFriendshipStatusesForCurrentUser(this.state.friendships),
      })
    }
