...
class IMFriendsScreen extends Component {
  ...
  static navigationOptions = ({ screenProps, navigation }) => {
    ...
    //let headerTitle = navigation.state.params.friendsScreenTitle || IMLocalized('γείτονες');
    let headerTitle =  IMLocalized('γείτονες');
    ...
    return {
      ...
      headerLeft: showDrawerMenuButton && (
        ...
        imageStyle={{position:'relative', left:-2, leftMargin:0, width:61, height:29, zIndex:100, }} 
        //imageStyle={{ tintColor: currentTheme.fontColor }}
          iconSource={appStyles.iconSet.menuHamburgerChat}
      ...
      headerTitleStyle: {
        ...
        position:'relative', 
        left:Math.floor(appStyles.WINDOW_WIDTH /2 - 120),
  ...
  constructor(props) {
    ...
    this.followEnabled = this.props.navigation.state.params.followEnabled || this.props.navigation.getParam('followEnabled');
  ...
  componentDidMount() {
    ...
    this.friendshipTracker = new FriendshipTracker(this.context.store, user.id || user.userID, this.followEnabled, this.followEnabled, this.followEnabled);
  ...
  updateFilteredFriendships = (keyword) => {
    ...
    const filteredFriendships = 
      filteredNonFriendshipsFromUsers(keyword, this.props.users, this.props.friendships)
      .filter(element => element.user && element.user.id != this.props.user.id);
  ...
  onFriendAction = (item, index) => {
    if (this.state.isLoading || (item.user && item.user.id == this.props.user.id)) {
      return;
    }
    switch (item.type) {
  ...
  render() {
    const emptyStateConfig = {
      title: IMLocalized("Δεν βρέθηκαν γείτονες"),
      description: IMLocalized("Οι νέοι γείτονες θα προστεθούν αυτόματα μόλις εγγραφούν στην εφαρμογή"),
